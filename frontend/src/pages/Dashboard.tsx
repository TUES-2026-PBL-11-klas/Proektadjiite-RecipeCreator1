...
import { ShoppingBasket, BookOpen, Sparkles, ArrowRight, ChefHat, Package } from 'lucide-react';
  ...
  return (
    <div className="page-wrapper">
      ...
      <div className="flex flex-col items-center text-center mb-8 pb-6 border-b border-border">
        ...
        <h1 className="text-3xl font-bold text-foreground font-serif leading-tight">Welcome back, Chef!</h1>
        <p className="text-sm text-muted-foreground mt-1.5">What would you like to cook today?</p>
      </div>

      {/* 3 Feature Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-5">
        {featureCards.map(({ to, icon: Icon, title, desc, cta, iconCls, ctaCls }) => (
          <Link
            key={to}
            to={to}
          ...
          >
            ...
            <h2 className="text-lg font-bold mb-1.5 text-foreground">{title}</h2>
            <p className="text-sm leading-relaxed flex-1 mb-4 text-muted-foreground">{desc}</p>
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-white/75">Let AI create a recipe from your pantry items — instantly.</p>
          </div>
        </div>
